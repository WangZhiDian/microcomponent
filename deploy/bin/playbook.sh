...
backup_dir=/opt/application/price_service
...
if [ ! $# -eq 1 ]; then
  echo "error info: need the env type:dev prod."
  exit
fi

env_type=$1
echo "the install env is ${env_type}"
...
sh ${current_work_dir}/install.sh --install ${env_type}
# 4 执行目标服务器解压后目录bin中的start脚本卸载服务
sh ${current_work_dir}/start.sh

# 5 拷贝上一次安装包到服务器历史备份目录
