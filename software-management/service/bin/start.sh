...
if [ -z "${process_id}" ];then
    ...
    source /etc/profile
    su service -c "nohup java -jar -Xmx${SERVICE_SERVER_XMX} -Djava.ext.dirs=${SERVICE_LIB_PATH} -Dspring.config.location=${SERVICE_CONF_PATH}/ -Dlogging.config=${SERVICE_CONF_PATH}/logback.xml ${SERVICE_JAR_PATH} 1>/dev/null 2>/dev/null &"
    cd ${CURRENT_DIR} 
    sleep 5
    process_id=`ps -ef|grep -i "${SERVICE_JAR_PATH}" |grep -v "grep"|awk '{print $2}'`
    if [ -z "${process_id}" ];then
